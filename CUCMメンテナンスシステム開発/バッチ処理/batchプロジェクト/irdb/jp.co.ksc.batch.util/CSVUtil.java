        ...
        for (int i = 0; i < dataMap.length; i++) {
            for (int j = 0; j < headerList.size(); j++) {
                ...
                buf.append(replaceStr(
                        dataMap[i].get(headerList.get(j).toString()),
                        sizeList.get(j)));
                buf.append(getQuotation());
        ...
        while((s = br.readLine()) != null){
            ...
            if (getHasHeader() && !read) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    // 囲み文字の消去
                    if (!"".equals(getQuotation())
                            && tmp.startsWith(getQuotation())
                            && tmp.endsWith(getQuotation())) {
                        tmp = tmp.substring(1, tmp.length() - 1);
                    }
                    headerList.add(tmp);
                }
            ...
            // 値行
                ...
                if (st.countTokens() != headerList.size()) {
                    throw new CSVException("ヘッダーの数と値の数が一致していません。[" + lineCnt + "]行目　headerList.size：[" + headerList.size() + "], countTokens：[" + st.countTokens() + "]");
                }
